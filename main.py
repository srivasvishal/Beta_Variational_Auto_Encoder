...
#     and saves it to `filename`.  Never passes a 3-D (100×64×64) tensor to imshow.
#     """
#     model.eval()
...
#                 img = recon.view(1, size, size)       # force (1, H, W)
#                 imgs.append(img)
#
#     # 2) cat into one batch: (n_z*n_samples, 1, H, W)
#     batch = torch.cat(imgs, dim=0)
#
#     # 3) build a single canvas: shape (C=1, H_total, W_total)
#     canvas = make_grid(batch, nrow=n_samples, normalize=True, pad_value=1)
#
#     # 4) remove channel dim → 2D array (H_total, W_total)
#     arr = canvas.squeeze(0).cpu().numpy()
#
#     # 5) plot & save
